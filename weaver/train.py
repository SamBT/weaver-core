...
parser.add_argument("--contrastive-mode", action='store_true', default=False,
                    help='run in contrastive mode if this flag is set; if no flag, run in classification mode')
...
parser.add_argument('--out-dim', type=int, default=8,
                    help='embedding dimension for contrastive/representation learning applications')
...
def save_root_embedding_space_contrastive(args, output_path, data_config, all_embed, all_project, all_labels):
    """
    Saves manifold and tangent space embeddings along with labels to a .root file.
    ...
    embeds = np.concatenate([tens.cpu().numpy() for tens in all_embed])
    projects = np.concatenate([tens.cpu().numpy() for tens in all_project])
    labels = np.concatenate([tensor.cpu().numpy() for tensor in all_labels])
    
    output['embeddings'] = embeds
    output['projections'] = projects
...
def _main(args):
    ...
    if training_mode:
        ...
        # training loop
        if args.regression_mode or args.embedding_mode or args.contrastive_mode:
        ...
        for name, get_test_loader in test_loaders.items():
            ...
            if args.model_prefix.endswith('.onnx'):
                ...
                if args.embedding_mode:
                    ...
                    all_embed,all_project,all_labels = evaluate(
                        model, test_loader, dev, epoch=None, for_training=False, tb_helper=tb, args = args)
                else:
            ...
            if args.predict_output:
                ...
                if output_path.endswith('.root'):
                    if args.embedding_mode:
                        output_path = base + '_embedding_' + name + ext
                        save_root_embedding_space(args, output_path, data_config, all_man_embed, all_tan_embed, all_labels)
                    if args.contrastive_mode:
                        output_path = base + '_contrastive_embedding_' + name + ext
                        save_root_embedding_space_contrastive(args, output_path, data_config, all_embed, all_project, all_labels)
                    else:
